...
public class PersonaTest {
	...
	public void testIdProgressiviPerPersoneTipoDiverso() {
		...
		//sfruttando le aggiunte precedenti
		new Attenta(this.ambiente).getId();
		new Distratta(this.ambiente).getId();
		new Distratta(this.ambiente).getId();
		assertEquals("Gli id sono progressivi base 0 per ciascun tipo dinamico!", 3, new Attenta(this.ambiente).getId());
		assertEquals("Gli id sono progressivi base 0 per ciascun tipo dinamico!", 4, new Distratta(this.ambiente).getId());
	}
