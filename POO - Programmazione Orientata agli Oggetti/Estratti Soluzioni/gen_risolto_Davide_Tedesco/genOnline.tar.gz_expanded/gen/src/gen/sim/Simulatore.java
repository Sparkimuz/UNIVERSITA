...
public class Simulatore {
	...
	private void creaPopolazione() {
		...
		for(int i=0; i<NUMERO_INIZIALE_PER_TIPOLOGIA; i++) {
			...
			//per generare 4 volte la popolazione delle altre specie
			//			for(int j=0; j<4; j++) {
	...
	public void simula() {
		...
		if (!fineSimulazioneRichiesta()) {
			...
			this.gui.getInterruptingThread().start();
			try {
	...
	private boolean isRiproduzionePossibile(Animale animale, Set<Animale> stessaPosizione) {
		if (stessaPosizione.size()!=2 || getNumeroTipologie(stessaPosizione)!=1) return false;
		final Iterator<Animale> it = stessaPosizione.iterator();
