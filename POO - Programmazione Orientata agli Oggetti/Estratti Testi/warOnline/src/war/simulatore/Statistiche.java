...
public class Statistiche {
	...
	public Map<Tank, Integer> distruttiPerTank(Set<Proiettile> letali) {
		...
		for(Proiettile p : letali) {

			if(tank2distrutti.containsKey(p.getLanciatore())) {
				int numeroDistrutti = tank2distrutti.get(p.getLanciatore());
				if(p.getDistrutto() != null)
					tank2distrutti.put(p.getLanciatore(), ++numeroDistrutti);
			}
			else {
				if(p.getDistrutto()!=null) {
					Integer num = 1;
					tank2distrutti.put(p.getLanciatore(), num);
				}
			}
		}
	...
	public Map<Fazione, Set<Proiettile>> vulnerabilitaPerTipo(Set<Proiettile> letali) {
		...
		Map<Fazione,Set<Proiettile>> tipo2vulnerabilita = new HashMap<Fazione, Set<Proiettile>>();
		
		for(Proiettile p : letali) {
			Fazione f = Factory.getFazione(p.getLanciatore().getClass());
			if(tipo2vulnerabilita.containsKey(f)) {
				tipo2vulnerabilita.get(f).add(p);
			}
			else {
				Set<Proiettile>proiettili = new HashSet<Proiettile>();
				proiettili.add(p);
				tipo2vulnerabilita.put(f, proiettili);
				
			}
		}
		
		
		return tipo2vulnerabilita;
	}
	...
	public List<Fazione> ordinaStrategieDiCombattimento(final Map<Fazione, Set<Proiettile>> c2p) {
		List<Fazione> classifica = new ArrayList<Fazione>(c2p.keySet());
		
		Collections.sort(classifica, new ComparatoreFazioni(c2p));
